...
const UploadForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/upload`, form, {
        headers: {
    ...
    if (file) {
      if (type === 'thumbnail') {
        if (formData.thumbnail) {
          setFileStatus({ ...fileStatus, thumbnail: 'Thumbnail updated' });
        ...
        if (formData.video) {
          setFileStatus({ ...fileStatus, video: 'Video updated' });
  ...
  const FileInput = ({ type, accept, onChange }) => (
    <div 
      className={`relative border-2 border-dashed rounded-lg p-8 text-center transition-all duration-200 
        ${dragActive[type] ? 'border-blue-500 bg-blue-500/10' : 'border-gray-600 hover:border-gray-500'}`}
    ...
    >
      ...
      <div className="flex flex-col items-center space-y-4">
        {type === 'thumbnail' && thumbnailPreview ? (
          <img src={thumbnailPreview} alt="Thumbnail preview" className="w-32 h-32 object-cover rounded" />
        ) : type === 'thumbnail' ? (
